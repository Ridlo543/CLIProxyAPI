...
func TestAttachResolvedOAuthModelInfoUsesGlobalAliasCapability(t *testing.T) {
	manager := NewManager(nil, nil, nil)
	...
	auth := &Auth{ID: "oauth", Provider: "claude", Attributes: map[string]string{"auth_kind": "oauth"}}
	result := manager.resolveOAuthModelAliasWithResult(auth, "public-model")
	req := manager.attachResolvedOAuthModelInfo(cliproxyexecutor.Request{}, auth, "shared-upstream", result)
	assertResolvedThinkingLevels(t, req, "high")
}

func TestAttachResolvedOAuthModelInfoUsesSelectedPerAuthOverride(t *testing.T) {
	manager := NewManager(nil, nil, nil)
	manager.SetConfig(&internalconfig.Config{OAuthModelAlias: map[string][]internalconfig.OAuthModelAlias{"claude": {{
		Name: "shared-upstream", Alias: "public-model",
	...
	manager.SetOAuthModelAlias(manager.runtimeConfigSnapshot().OAuthModelAlias)
	authHigh := &Auth{ID: "oauth-high", Provider: "claude", Attributes: map[string]string{"auth_kind": "oauth"}}
	authMax := &Auth{ID: "oauth-max", Provider: "claude", Attributes: map[string]string{"auth_kind": "oauth"}}
	SetOAuthModelAliasesAttribute(authHigh, []internalconfig.OAuthModelAlias{{Name: "shared-upstream", Alias: "public-model", Thinking: &registry.ThinkingSupport{Levels: []string{"high"}}}})
	SetOAuthModelAliasesAttribute(authMax, []internalconfig.OAuthModelAlias{{Name: "shared-upstream", Alias: "public-model", Thinking: &registry.ThinkingSupport{Levels: []string{"max"}}}})
	resultHigh := manager.resolveOAuthModelAliasWithResult(authHigh, "public-model")
	resultMax := manager.resolveOAuthModelAliasWithResult(authMax, "public-model")
	assertResolvedThinkingLevels(t, manager.attachResolvedOAuthModelInfo(cliproxyexecutor.Request{}, authHigh, "shared-upstream", resultHigh), "high")
	assertResolvedThinkingLevels(t, manager.attachResolvedOAuthModelInfo(cliproxyexecutor.Request{}, authMax, "shared-upstream", resultMax), "max")
}

func TestAttachResolvedOAuthModelInfoUsesPerAuthBaseFallbackBeforeGlobalExactSuffix(t *testing.T) {
	manager := NewManager(nil, nil, nil)
	manager.SetConfig(&internalconfig.Config{OAuthModelAlias: map[string][]internalconfig.OAuthModelAlias{"claude": {{
		Name: "global-upstream", Alias: "public(low)",
		Thinking: &registry.ThinkingSupport{Levels: []string{"low"}},
	}}}})
	manager.SetOAuthModelAlias(manager.runtimeConfigSnapshot().OAuthModelAlias)
	auth := &Auth{ID: "oauth-selected", Provider: "claude", Attributes: map[string]string{"auth_kind": "oauth"}}
	SetOAuthModelAliasesAttribute(auth, []internalconfig.OAuthModelAlias{{
		Name: "selected-upstream", Alias: "public",
		Thinking: &registry.ThinkingSupport{Levels: []string{"high"}},
	}})
	result := manager.resolveOAuthModelAliasWithResult(auth, "public(low)")
	if result.UpstreamModel != "selected-upstream(low)" {
		t.Fatalf("routing result = %q, want selected-upstream(low)", result.UpstreamModel)
	}
	req := manager.attachResolvedOAuthModelInfo(cliproxyexecutor.Request{}, auth, result.UpstreamModel, result)
	assertResolvedThinkingLevels(t, req, "high")
}

func TestAttachResolvedOAuthModelInfoClearsStaleOAuthCapability(t *testing.T) {
	stale := &registry.ModelInfo{ID: "stale", Thinking: &registry.ThinkingSupport{Levels: []string{"high"}}}
	req := cliproxyexecutor.Request{Metadata: map[string]any{resolvedAPIKeyModelInfoMetadataKey: stale, "keep": "value"}}
	manager := NewManager(nil, nil, nil)
	oauth := &Auth{ID: "oauth", Provider: "claude", Attributes: map[string]string{"auth_kind": "oauth"}}
	cleared := manager.attachResolvedOAuthModelInfo(req, oauth, "upstream", OAuthModelAliasResult{UpstreamModel: "upstream"})
	if _, ok := ResolvedAPIKeyModelInfo(cleared); ok {
		t.Fatal("stale OAuth capability was not cleared")
	}
	if cleared.Metadata["keep"] != "value" {
		t.Fatalf("unrelated metadata was not preserved: %#v", cleared.Metadata)
	}
	apiKey := &Auth{ID: "api", Provider: "claude", Attributes: map[string]string{"auth_kind": "apikey"}}
	if info, ok := ResolvedAPIKeyModelInfo(manager.attachResolvedOAuthModelInfo(req, apiKey, "upstream", OAuthModelAliasResult{})); !ok || info != stale {
		t.Fatal("API-key capability metadata should be preserved")
	}
}
