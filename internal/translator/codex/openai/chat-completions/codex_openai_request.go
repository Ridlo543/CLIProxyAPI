...
func ConvertOpenAIRequestToCodex(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	if strings.HasSuffix(upstreamModel, "-cyber") {
		base := strings.TrimSuffix(upstreamModel, "-cyber")
		if base == "gpt-6" {
			upstreamModel = "gpt-6-astra"
		} else if base == "gpt-5.6" {
					...
					for j := 0; j < len(items); j++ {
						...
						case "image_url", "input_image":
							...
							if role == "user" {
								...
								u := it.Get("image_url.url").String()
								if u == "" {
									u = it.Get("image_url").String()
								}
								if u == "" {
									u = it.Get("url").String()
								}
								...
								d := it.Get("image_url.detail").String()
								if d == "" {
									d = it.Get("detail").String()
								}
								if d != "" {
									part, _ = sjson.SetBytes(part, "detail", d)
								}
								contentItems = append(contentItems, part)
							}
						case "file":
